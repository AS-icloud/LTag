...
27) Default window layout (front + back) verified: front QR top-left / text lower-right, back
   mirrored (text upper area / QR lower-left) — checked against your own printed PDF and
   confirmed no overlap (0.5mm clear gap between QR and text on both panels) and a true mirror
   across the fold. No position changes were needed or made. IMPORTANT: there's no autosave —
   if you drag/resize windows directly in the live app and then refresh without pressing
   "💾 Save" first, it reverts to these defaults and your edits are lost. Click Save before
   refreshing, or before closing the tab, any time you've made changes you want to keep.
