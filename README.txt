...
════════════════════════════════════════════════════════════
UPDATE — v37, 13 Aug 2026
════════════════════════════════════════════════════════════

1. CLIPBOARD IS NOW PLAIN TEXT ONLY
   The "Copy All to Clipboard" button on the info page no longer puts a rich
   text/html flavour on the clipboard — only text/plain. That is what guarantees
   an unformatted paste into Notes, WhatsApp, Word, Mail and plain editors: a
   rich flavour is what those apps latch onto to apply their own styling and
   their own data detectors.

   Field labels are UPPERCASE as the visual substitute for bold.

   Phone, mobile, WhatsApp, email, address and website values additionally carry an
   invisible SOFT HYPHEN (U+00AD) after each digit group and before @ and dots. This is
   the noDetect() function copied VERBATIM from the Address Label app, which is proven
   in real use on iPhone. Apple Notes, WhatsApp, Word and Outlook all re-scan pasted
   content and auto-link anything that still looks like a phone number, email or
   address — the soft hyphen breaks that pattern match.

   It is applied once per digit GROUP, not per character. That matters: "+44 7762 609490"
   picks up only 3 invisible characters, so the value stays clean, and a number copied
   back out of Notes still pastes into the iPhone dialler and dials correctly.

   Applied to: viewer.html, info-ankitsrivastava.html, multilingual.html and
   the "Copy original / Copy translated" buttons in the designer itself.

   The tel: / mailto: / wa.me links shown ON the info page are unaffected —
   those are still fully tappable. This only changes what lands on the clipboard.

2. VIRTUAL PAPER — LABEL STRIP (A4 / A5 / A6)
   Ported from the Address Label app, for printing a small tag on a big sheet.

   Print & Export -> Paper mode: Virtual -> "Label strip edge":
     Off      - unchanged behaviour; Position-on-page + margins as before
     Auto     - Left for portrait sheets, Top for landscape
     Left     - strip on the left edge, feed gap measured from the top
     Right    - strip on the right edge, feed gap measured from the top
     Top      - strip on the top edge, feed gap from the left, design rotated 90 deg
     Bottom   - strip on the bottom edge, feed gap from the left, rotated 90 deg

   "Feed gap (mm)" is the blank run-in before the strip so the printer's pickup
   rollers have something to grip. Default 20 mm.

   While a strip edge is active, Position-on-page and the four margin fields are
   ignored (and greyed out) — the strip is anchored hard to the chosen sheet edge.
   The design keeps its true mm size; it is never scaled to the sheet.

   Export the PDF/PNG/JPG as usual, then print at 100% / Actual size with
   "Fit to page" DISABLED.

3. VERSION BUMPED TO v37 — NO MORE STALE CACHES
   The folder, the app title, the manifest name and the service-worker cache name are
   all now v37. Specifically:

     - sw.js CACHE_NAME is 'luggage-tag-designer-v37-r1'. On activate the worker
       deletes every cache that isn't that name, so all v36-era files are evicted.
     - index.html declares APP_VERSION = '37' and appends ?v=37 to the service-worker
       registration, the manifest link and the viewer.html fetch, so the browser's own
       HTTP cache can't serve the old copies either.
     - The worker is registered with updateViaCache:'none' — without that, a browser can
       keep serving a cached sw.js for up to 24 hours and never notice a new release.
     - A new worker now skip-waits and the page reloads itself once, so you land on the
       new build immediately instead of after two visits.
     - info-ankitsrivastava.html was missing from the service worker's precache list and
       has been added.

   WHEN YOU DEPLOY A FUTURE VERSION: bump BOTH the APP_VERSION constant near the bottom
   of index.html AND the CACHE_NAME string at the top of sw.js. Either one alone will
   leave some users on stale files.

4. TEXT AROUND A WINDOW'S PERIMETER (circle, square AND rectangle)
   Ported from the Address Label app's circular note-window text, and extended so it
   works on straight-edged windows too.

   On any non-QR window: tick "Run the text around this window's perimeter".

     Start (%)        spins the run around the outline. 0% centres it on the top edge.
     Inset (mm)       pulls the text inward, away from the border.
     Letter gap (mm)  extra tracking — raise it to spread a short word right round.
     Flip             runs anticlockwise with the letters turned over, so the text
                      reads from the inside of the shape instead of the outside.

   On a CIRCLE the text curves round the arc, exactly as in the address label app.
   On a SQUARE or RECTANGLE the text genuinely turns the corners — it flows along the
   top edge, rounds the corner and continues down the side as one continuous run. It is
   not one separate line per edge.

   Notes:
     - Line breaks in the text are ignored in this mode; the string flows as one run.
     - The Align setting and the image/text position setting don't apply on a perimeter.
     - The window's own Rotation (deg) field still works and turns the whole ring with it.
     - Implemented with per-character transforms rather than SVG <textPath>, because the
       PNG/JPG/PDF export path draws the serialised SVG through an <img>, where textPath
       support is unreliable. This way the preview, the print stage and every exported
